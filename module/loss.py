...
class DetectorLoss(nn.Module):
    ...
    def build_target(self, preds, targets):
        ...
        # Use the center point of each grid as the reference point for bounding box regression
        quadrant = torch.tensor([[0, 0], [1, 0], 
                                 [0, 1], [1, 1]], device=self.device)

        if targets.shape[0] > 0:
            # Map coordinates to the feature map scale
            scale = torch.ones(6).to(self.device)
            scale[2:] = torch.tensor(preds.shape)[[3, 2, 3, 2]]
            gt = targets * scale

            # Expand dimensions and copy data
            gt = gt.repeat(4, 1, 1)
            ...
            # Indices of foreground positions
            gi, gj = gij[j].T
    ...
    def forward(self, preds, targets):
        ...
        # Define loss functions for obj and cls
        BCEcls = nn.NLLLoss() 
        # Smooth L1 works best compared to BCE
        ...
        if len(gt_box) > 0:
            # Calculate detection box regression loss
            b, gx, gy = ps_index[0]
            ...
            # Calculate detection box IOU loss
            ...
            # Calculate loss for target class classification branch
        ...
        # Calculate loss for foreground/background classification branch
